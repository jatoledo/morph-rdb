...
abstract class MorphBaseRunnerFactory {
	...
	/**
	 * createRunner : configurationDirectory:String , configurationFile:String
	 */
	...
	/**
	 * createRunner: properties:MorphProperties
	 */
	...
	/**
	 * Cli : Runner with mapping
	 */
	def createRunner(properties:MorphProperties,mappingFilePath:String,query:String, outputFile:String):MorphBaseRunner={
	    
			val morphProperties = properties.asInstanceOf[MorphProperties];

			//BUILDING CONNECTION
			val conn = this.createConnection(morphProperties);
			val runner = this.createRunner(conn, properties, mappingFilePath, query, outputFile);
			runner;


	}
	/**
	 * createRunner : by Jhon Toledo (CLI vsersion)
	 */
	def createRunner(connection:Connection, properties:MorphProperties, mappingFilePath:String, queryPath:String, outputFilePath:String):MorphBaseRunner = {
			...
			/*commented by Jhon: here is mirror */ 
			...
			val mappingDocumentFile = try {
			  //if(morphProperties.mappingDocumentFilePath == null) {
				...
				else { /*morphProperties.mappingDocumentFilePath;*/ mappingFilePath;}
			}
			catch { case e:Exception => { /*morphProperties.mappingDocumentFilePath;*/mappingFilePath; } }
 
			
			val mappingDocument = this.readMappingDocumentFile(mappingFilePath
			...
			val writer:Writer = if(!outputFilePath.isEmpty()/*!morphProperties.outputFilePath.isDefined && outputFile!= null*/) {
			   val outputFileName = outputFilePath;
			...
			val outputStream:OutputStream = if(!outputFilePath.isEmpty()/*morphProperties.outputFilePath.isDefined*/) {
			  //val outputFileName = morphProperties.outputFilePath.get;
				val outputFileName = outputFilePath
			...
			//TODO Jhon T
			//Comprobar si hay query
			//si no hay query quer no haga query trasnlater
	 ...
	 * Jhon T
	 */
	def createConnection(driverString:String,url:String,username:String, password:String): Connection ={
			val connection ={
					DBUtility.getLocalConnection2(driverString:String,url:String,username:String, password:String);
			}
			connection
	}
